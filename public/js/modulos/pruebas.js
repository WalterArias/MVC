const listarClientes = function () {
  let tabla1 = $("#tablaClientes").DataTable({
    ...
    columns: [{
      ...
      {
        defaultContent: "<button type='button' class ='AgregarCliente btn btn-sm btn-light' id = 'AgregarCliente' data-toggle='tooltip' data-placement='top' title='Selecciona Cliente'> <i class='bi bi-check2-square'></i></button>",
      },
    ],
    columnDefs: [{
        width: "20%",
...
const listarProductos = function () {
  let tabla2 = $("#tablaArticulos").DataTable({
    ajax: {
    ...
    columns: [{
      ...
      {
        defaultContent: "<button type='button' class =' AgregarArticulo btn btn-sm btn-light'  id = 'AgregarArticulo' data-toggle='tooltip' data-placement='top' title='Selecciona Articulo'> <i class='bi bi-check2-square'></i></button>",
      },
    ],
    columnDefs: [{
        width: "20%",
...
const agregarArticulo = function (tbody, table) {
  $("#tablaArticulos tbody").on("click", "button", function () {
    ...
    let fila =
      `<tr class = "filas" id = "fila${contadorFila}"><td class="col-sm-8" ><input type = "text" name ="idArticulo[]"  size="9" readonly value ="${dato.idarticulo}"></td><td> <input type = "text" name ="descripcion[]" readonly value ="${dato.descripcion}"></td><td class="col-sm-4"> <input type = "text"  name ="cantidad[]"   size="3" value ="${+cantidad}"></td><td> <input type = "text" name ="precio[]" id ="precio" size="9" pattern="^$d{1,3}(,d{3})*(.d+)?$" readonly value ="${dato.precio}"></td><td><input type = "text" name ="subtotal[]" id ="subtotal" pattern="^$d{1,3}(,d{3})*(.d+)?$" size="9" readonly value ="${subtotal}"></td><td> <button type='button' id = 'actualizar' onclick ='calcularSubtotalFila()' class='btn btn-secondary btn-sm'><i class='bi bi-arrow-repeat'></i></button></td><td> <button type='button' id = 'eliminaLinea' onclick ='eliminarFila(${contadorFila})' class='btn btn-secondary btn-sm'><i class='bi bi-trash'></i></button></td></tr>`;
    contadorFila++;
    $("#detalle").append(fila);
